...
class TrainingTab(QWidget):
    ...
    def setup_ui(self, main_layout):
        ...
        self.task_combo.addItems(["目标检测 (Detection)", "实例分割 (Segmentation)", "图像分类 (Classification)"])
        ...
        self.generate_dataset_btn = QPushButton("📁 一键生成数据集文件夹结构")
        ...
        self.workers_spin.setValue(0)  # 默认0避免多线程问题
        self.workers_spin.setToolTip("数据加载线程数。0=禁用多线程(最稳定)，1-8=多线程加速")

        # Patience (早停耐心值)
        self.patience_spin = QSpinBox()
        self.patience_spin.setRange(1, 200)
        self.patience_spin.setValue(50)
        self.patience_spin.setToolTip("早停耐心值。连续多少轮无改善后停止训练")

        # 🎛️ 重要开关参数
        # 验证开关 (关键!)
        self.enable_val_checkbox = QCheckBox("启用验证")
        self.enable_val_checkbox.setChecked(False)  # 默认关闭避免内存冲突
        self.enable_val_checkbox.setToolTip("是否在训练过程中进行验证。关闭可避免内存冲突")
        self.enable_val_checkbox.setStyleSheet("QCheckBox { color: #FF6B6B; font-weight: bold; }")

        # 绘图开关 (关键!)
        self.enable_plots_checkbox = QCheckBox("生成训练图表")
        self.enable_plots_checkbox.setChecked(False)  # 默认关闭避免matplotlib问题
        self.enable_plots_checkbox.setToolTip("是否生成训练过程图表。关闭可避免内存问题")
        self.enable_plots_checkbox.setStyleSheet("QCheckBox { color: #FF6B6B; font-weight: bold; }")

        # AMP开关
        self.enable_amp_checkbox = QCheckBox("自动混合精度(AMP)")
        self.enable_amp_checkbox.setChecked(False)  # 默认关闭提高稳定性
        self.enable_amp_checkbox.setToolTip("启用自动混合精度训练。可提升速度但可能影响稳定性")

        # 缓存设置
        self.cache_combo = QComboBox()
        self.cache_combo.addItems(["False (禁用)", "True (RAM缓存)", "disk (磁盘缓存)"])
        self.cache_combo.setCurrentText("False (禁用)")
        self.cache_combo.setToolTip("数据缓存方式。RAM最快但占内存，磁盘较慢但稳定，禁用最稳定")

        # 详细日志开关
        self.verbose_checkbox = QCheckBox("详细日志")
        self.verbose_checkbox.setChecked(True)
        self.verbose_checkbox.setToolTip("是否输出详细的训练日志信息")
        
        # Add widgets to form layout - 基础参数
        ...
        # 🔧 新增关键参数
        model_layout.addRow("数据加载线程:", self.workers_spin)
        model_layout.addRow("早停耐心值:", self.patience_spin)

        # 缓存设置
        model_layout.addRow("数据缓存:", self.cache_combo)

        # 🎛️ 创建关键开关参数组
        switches_group = QGroupBox("🔧 关键开关参数 (影响稳定性)")
        switches_layout = QVBoxLayout()

        # 添加重要提示
        warning_label = QLabel("⚠️ 以下参数直接影响训练稳定性，建议保持默认设置")
        warning_label.setStyleSheet("QLabel { color: #FF6B6B; font-weight: bold; background: #2a2a2a; padding: 5px; border-radius: 3px; }")
        switches_layout.addWidget(warning_label)

        # 添加开关控件
        switches_layout.addWidget(self.enable_val_checkbox)
        switches_layout.addWidget(self.enable_plots_checkbox)
        switches_layout.addWidget(self.enable_amp_checkbox)
        switches_layout.addWidget(self.verbose_checkbox)

        switches_group.setLayout(switches_layout)
        model_layout.addWidget(switches_group)
    ...
    def start_training(self):
        ...
        enable_val = self.enable_val_checkbox.isChecked()
        enable_plots = self.enable_plots_checkbox.isChecked()
        enable_amp = self.enable_amp_checkbox.isChecked()
        verbose = self.verbose_checkbox.isChecked()

        # 处理缓存设置
        cache_text = self.cache_combo.currentText()
        if "False" in cache_text:
            cache = False
        elif "True" in cache_text:
            cache = True
        ...
        other_args.update({
            ...
            'verbose': verbose,
            'cache': cache,
            'lr0': self.lr_spin.value()  # 确保学习率也被传递
        })

        # 记录关键参数设置
        self.log_message(f"🔧 关键参数设置:")
        self.log_message(f"   Workers: {workers} (数据加载线程)")
        self.log_message(f"   Patience: {patience} (早停耐心值)")
        self.log_message(f"   Validation: {enable_val} (验证开关)")
        self.log_message(f"   Plots: {enable_plots} (绘图开关)")
        self.log_message(f"   AMP: {enable_amp} (混合精度)")
        self.log_message(f"   Cache: {cache} (数据缓存)")
        self.log_message(f"   Verbose: {verbose} (详细日志)")
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.enable_val_checkbox.setEnabled(enabled)
        self.enable_plots_checkbox.setEnabled(enabled)
        self.enable_amp_checkbox.setEnabled(enabled)
        self.verbose_checkbox.setEnabled(enabled)
        self.cache_combo.setEnabled(enabled)
    ...
    def update_settings(self, settings):
        ...
        self.lr_spin.setValue(settings.get('learning_rate', 0.01))

        # 🔧 新增参数的设置加载
        self.workers_spin.setValue(settings.get('workers', 0))
        self.patience_spin.setValue(settings.get('patience', 50))
        self.enable_val_checkbox.setChecked(settings.get('enable_val', False))
        self.enable_plots_checkbox.setChecked(settings.get('enable_plots', False))
        self.enable_amp_checkbox.setChecked(settings.get('enable_amp', False))
        self.verbose_checkbox.setChecked(settings.get('verbose', True))

        # 缓存设置
        cache_setting = settings.get('cache', False)
        if cache_setting is False:
            self.cache_combo.setCurrentText("False (禁用)")
        elif cache_setting is True:
            self.cache_combo.setCurrentText("True (RAM缓存)")
        elif cache_setting == "disk":
            self.cache_combo.setCurrentText("disk (磁盘缓存)")
        else:
            self.cache_combo.setCurrentText("False (禁用)")
    ...
    def update_model_list(self):
        ...
        if self.task_type == "detect":
            ...
            models = detect_models + obb_models
        ...
        if models:
            ...
            if self.task_type == "detect":
                ...
                default_model = "yolo11n-seg"  # 使用最新的YOLO11分割模型
            elif self.task_type == "classify":
        ...
        if self.task_type == "detect":
            ...
            self.log_message("💡 推荐：YOLO11-seg系列（最新）> YOLO8-seg系列（稳定）> YOLO5-seg系列（经典）")
        elif self.task_type == "classify":
    ...
    def on_task_type_changed(self, index):
        ...
        if old_task_type != self.task_type:
            ...
            if self.task_type == "classify":
                ...
                QMessageBox.information(self, "实例分割任务数据集结构说明",
                    "实例分割任务数据集要求如下：\n\n"
                    "1. 训练/验证集分别有 images 和 labels 两个文件夹。\n"
                    "2. images/ 下为图片，labels/ 下为同名 txt 文件（分割标注格式）。\n"
                    "3. 标注格式：class_id x1 y1 x2 y2 ... xn yn（多边形坐标）\n"
                    "4. 根目录需有 data.yaml 配置文件。\n\n"
                    "示例：\n"
                    "dataset/\n  images/\n    train/\n      xxx.jpg\n    val/\n      yyy.jpg\n  labels/\n    train/\n      xxx.txt\n    val/\n      yyy.txt\n  data.yaml\n"
                    "\n支持多边形分割标注和RLE格式"
                )
    ...
    def try_create_data_yaml(self):
        """在检测和分割任务下，若未检测到data.yaml则自动生成一个模板。"""
        if self.task_type not in ["detect", "segment"]:
            return
        # 推断根目录
        ...
        task_name = "分割" if self.task_type == "segment" else "检测"
        QMessageBox.information(self, f"已自动生成data.yaml", f"已在{os.path.dirname(root_dir)}生成{task_name}任务的data.yaml，类别名：{names if names else ['class0']}\n如需修改请手动编辑data.yaml或classes.txt。")

    def update_task_specific_ui(self):
        """Update UI elements specific to detection, segmentation or classification tasks."""
        task_names = {
            "detect": "目标检测",
            "segment": "实例分割", 
            "classify": "图像分类"
        ...
        is_classification = self.task_type == "classify"
        has_labels = self.task_type in ["detect", "segment"]
        
        # 更新数据集组标题
        self.data_group.setTitle(f"数据集 ({task_names.get(self.task_type, self.task_type)})")
        ...
        else:  # detect or segment
            self.train_images_label.setText("训练图像目录:")
            if self.task_type == "segment":
                self.train_labels_label.setText("训练标签目录 (分割标注):")
                self.val_labels_label.setText("验证标签目录 (分割标注):")
            else:  # detect
        ...
        # 控制标签相关控件的可用性
        self.train_labels_edit.setEnabled(has_labels)
        self.train_labels_btn.setEnabled(has_labels)
        self.train_labels_open_btn.setEnabled(has_labels)
        self.val_labels_edit.setEnabled(has_labels)
        self.val_labels_btn.setEnabled(has_labels)
        self.val_labels_open_btn.setEnabled(has_labels)
        
        # 更新微调模式文本
        if self.task_type == "classify":
            self.fine_tuning_mode.setText("微调模式（例如，冻结部分层，仅训练分类器）")
        elif self.task_type == "segment":
            self.fine_tuning_mode.setText("微调模式（冻结主干网络，仅训练分割头）")
        else:  # detect
            self.fine_tuning_mode.setText("微调模式（冻结主干网络，仅训练检测头）")
        
        # Enable fine-tuning for detection and segmentation
        self.fine_tuning_mode.setEnabled(has_labels)
    ...
    def generate_dataset_structure(self):
        """一键生成数据集文件夹结构，根据任务类型生成对应格式"""
        try:
            # 选择根目录
            root_dir = QFileDialog.getExistingDirectory(self, "选择数据集根目录（将在此目录下创建数据集结构）")
            if not root_dir:
                return
            
            # 根据任务类型确定数据集名称
            task_names = {
                "detect": "yolo_detection_dataset",
                "segment": "yolo_segmentation_dataset", 
                "classify": "yolo_classification_dataset"
            }
            
            dataset_name = task_names.get(self.task_type, "yolo_dataset")
            ...
            if self.task_type == "classify":
                ...
                created_dirs.extend([train_dir, val_dir])
                ...
                for class_name in example_classes:
                    ...
                    created_dirs.extend([train_class_dir, val_class_dir])
                    self.log_message(f"✅ 创建类别目录: {class_name}")
                
                # 创建类别说明文件
                classes_file = os.path.join(dataset_path, "classes.txt")
                with open(classes_file, 'w', encoding='utf-8') as f:
                    f.write("# 分类任务类别名称定义\n")
                    f.write("# 每行一个类别名称，与文件夹名称对应\n")
                    f.write("# 例如:\n")
                    for class_name in example_classes:
                ...
                # 自动填充路径到界面
                self.train_images_edit.setText(train_dir)
                self.val_images_edit.setText(val_dir)
                
                # 创建分类任务README
                readme_content = self.create_classification_readme(dataset_name, example_classes)
                
            else:
                # 检测和分割任务：创建YOLO格式的文件夹结构
                task_label = "分割" if self.task_type == "segment" else "检测"
                self.log_message(f"🔨 创建{task_label}任务数据集结构...")
                ...
                # 创建检测/分割任务README
                readme_content = self.create_detection_segmentation_readme(dataset_name, self.task_type)
            ...
            # 显示成功消息
            task_name = {"detect": "目标检测", "segment": "实例分割", "classify": "图像分类"}.get(self.task_type, self.task_type)
            success_msg = f"✅ {task_name}数据集结构创建成功！\n\n"
            success_msg += f"📁 数据集路径: {dataset_path}\n\n"
            success_msg += f"🔧 已自动填充界面路径，接下来请：\n"
            
            if self.task_type == "classify":
                success_msg += f"1. 将不同类别的图片放入对应的类别文件夹中\n"
                success_msg += f"2. 编辑classes.txt文件定义类别名称\n"
                success_msg += f"3. 确保训练集和验证集都有相同的类别文件夹\n\n"
            else:
    ...
    def create_classification_readme(self, dataset_name, example_classes):
        """创建分类任务的README内容"""
        return f"""# {dataset_name} - 图像分类数据集

## 目录结构
```
{dataset_name}/
├── train/                 # 训练集
│   ├── {example_classes[0]}/           # 类别1图片
│   ├── {example_classes[1]}/           # 类别2图片
│   └── {example_classes[2]}/           # 类别3图片
├── val/                   # 验证集
│   ├── {example_classes[0]}/           # 类别1图片
│   ├── {example_classes[1]}/           # 类别2图片
│   └── {example_classes[2]}/           # 类别3图片
...
1. 将不同类别的图片分别放入对应的类别文件夹中
2. 训练集和验证集都需要有相同的类别文件夹结构
3. 编辑 `classes.txt` 文件，定义你的类别名称
4. 确保每个类别都有足够的训练样本
...
## 示例
如果你要分类猫和狗：
- 将所有猫的图片放入 `train/cat/` 和 `val/cat/`
- 将所有狗的图片放入 `train/dog/` 和 `val/dog/`
- 在 `classes.txt` 中写入：
  ```
  cat
  dog
...
"""
    ...
    def create_detection_segmentation_readme(self, dataset_name, task_type):
        """创建检测/分割任务的README内容"""
        task_label = "实例分割" if task_type == "segment" else "目标检测"
        label_format = "分割标注" if task_type == "segment" else "YOLO格式"
        
        if task_type == "segment":
            label_example = """## 分割标注格式示例
每行一个目标，格式为：`class_id x1 y1 x2 y2 ... xn yn`
- class_id: 类别ID（从0开始）
- x1,y1,x2,y2,...: 多边形顶点坐标（相对于图像尺寸的比例，0-1）

示例标注文件内容：
```
0 0.1 0.2 0.3 0.2 0.3 0.8 0.1 0.8
1 0.5 0.1 0.9 0.1 0.9 0.4 0.5 0.4
```"""
        else:
            label_example = """## YOLO标注格式示例
每行一个目标，格式为：`class_id x_center y_center width height`
...
```"""
        ...
        return f"""# {dataset_name} - {task_label}数据集
...
"""
    ...
    def apply_safe_config(self, dialog):
        ...
        if reply == QMessageBox.Yes:
            ...
            # 🔧 安全模式的关键参数设置
            self.workers_spin.setValue(0)  # 禁用多线程
            self.patience_spin.setValue(50)
            self.enable_val_checkbox.setChecked(False)  # 关闭验证
            self.enable_plots_checkbox.setChecked(False)  # 关闭绘图
            self.enable_amp_checkbox.setChecked(False)  # 关闭AMP
            self.cache_combo.setCurrentText("False (禁用)")  # 禁用缓存
            self.verbose_checkbox.setChecked(True)  # 保持详细日志
    ...
    def apply_high_performance_preset(self):
        ...
        # 🔧 高性能模式的关键参数设置
        self.workers_spin.setValue(4)  # 适度多线程加速
        self.patience_spin.setValue(30)  # 较短耐心值快速收敛
        self.enable_val_checkbox.setChecked(True)  # 启用验证监控性能
        self.enable_plots_checkbox.setChecked(False)  # 关闭绘图节省时间
        self.enable_amp_checkbox.setChecked(True)  # 启用AMP加速
        self.cache_combo.setCurrentText("True (RAM缓存)")  # RAM缓存最快
        self.verbose_checkbox.setChecked(False)  # 减少日志输出
    ...
    def apply_ultra_safe_mode(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            # 🔧 超安全模式的关键参数设置
            self.workers_spin.setValue(0)  # 完全禁用多线程
            self.patience_spin.setValue(100)  # 更长耐心值
            self.enable_val_checkbox.setChecked(False)  # 关闭验证避免冲突
            self.enable_plots_checkbox.setChecked(False)  # 关闭绘图避免matplotlib问题
            self.enable_amp_checkbox.setChecked(False)  # 关闭AMP提高稳定性
            self.cache_combo.setCurrentText("False (禁用)")  # 完全禁用缓存
            self.verbose_checkbox.setChecked(True)  # 保持详细日志便于调试
