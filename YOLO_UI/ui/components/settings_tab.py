    ...
    def run(self):
        try:
            ...
            if cuda_available:
                ...
                message = f"CUDA 状态: 可用 (检测到 {device_count} 个 GPU)\nGPU: {device_name}"
                self.status_updated.emit(message, "green")
            else:
                self.status_updated.emit("CUDA 状态: 不可用", "red")
        except Exception as e:
            self.status_updated.emit(f"CUDA 状态: 检测失败 ({str(e)})", "orange")
...
class SettingsTab(QWidget):
    ...
    def check_cuda_status(self):
        """检查 CUDA 状态并更新显示 - 异步执行"""
        # 设置初始状态
        self.cuda_status_label.setText("CUDA 状态: 检测中...")
        self.cuda_status_label.setStyleSheet("color: orange;")
        
        # 创建并启动异步检测线程
        self.cuda_check_thread = CUDACheckThread()
        self.cuda_check_thread.status_updated.connect(self.update_cuda_status)
        self.cuda_check_thread.start()
    
    def update_cuda_status(self, message, color):
        """更新CUDA状态显示"""
        self.cuda_status_label.setText(message)
        self.cuda_status_label.setStyleSheet(f"color: {color};")
