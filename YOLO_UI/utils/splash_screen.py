...
def showSplashScreen(app, main_window):
    ...
    for i, (progress, message) in enumerate(steps):
        QTimer.singleShot(i * 100, lambda p=progress, m=message: splash.updateProgress(p, m))
    
    # 在所有步骤完成后显示主窗口，减少等待时间
    QTimer.singleShot(len(steps) * 100 + 50, lambda: finishSplash(splash, main_window))
